...
export = (config: scribe.Config) => {
    ...
    function writePostmanCollectionFile(groupedEndpoints: { [groupName: string]: scribe.Endpoint[] }, outputPath: string) {
    ...
    function makePostmanCollection(groupedEndpoints: { [groupName: string]: scribe.Endpoint[] }) {
        const collection: CollectionDefinition & { info: { description: string, schema: string, _postman_id: string } } = {
                ...
                info: {
                    name: config.title,
                    description: config.postman.description || '',
        ...
        if (config.postman.auth) {
            collection.auth = config.postman.auth;
        }
    ...
    function getAuthHeaderName() {
        const postmanAuth = config.postman.auth;
        ...
        switch (postmanAuth.type) {
            ...
            case 'apikey':
                const spec = postmanAuth.apikey;

                if (spec[0].type !== 'header') {
                    return null;
                }
                return spec[0].key;
            default:
